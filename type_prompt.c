...
void type_prompt( char *prompt)
{
	...
	if (gethostname(h_name, Max_name) == 0)
		snprintf(prompt, Max_all, "[sish] %s@%s:",pwd->pw_name, h_name);
	else
		snprintf(prompt, Max_all, "[sish] %s@unknown:", pwd->pw_name);

	length = strlen(prompt);

	if(strlen(p_name) < strlen(pwd->pw_dir) || strncmp(p_name, pwd->pw_dir, strlen(pwd->pw_dir)) != 0)
		snprintf(prompt + length, Max_all, "%s", p_name);
	else
		snprintf(prompt + length, Max_all, "~%s", p_name+strlen(pwd->pw_dir));
	
	length = strlen(prompt);

	if(geteuid() == 0)
		snprintf( prompt + length, Max_all,  "#");
	else
		snprintf( prompt + length, Max_all,  "$");
	return;
}
...
{
	char *prompt = NULL;
	prompt = (char *)malloc( Max_all*sizeof(char));
	type_prompt( prompt);
	fprintf( stdout, "%s\n", prompt);
	return 0;
}
